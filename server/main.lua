...
function depositMoney(identifier, amount)
    if amount <= 0 then
        TriggerClientEvent('apx-money:notify', source, "You cannot deposit $0 or less.")
        return
    end
    Apex.Functions.getMoney(identifier, 'cash', function(cash)
        if cash >= amount then
            ...
            TriggerClientEvent('apx-money:notify', source, "You do not have enough cash.")
        end
    end)
end
...
function withdrawMoney(identifier, amount)
    if amount <= 0 then
        TriggerClientEvent('apx-money:notify', source, "You cannot withdraw $0 or less.")
        return
    end
    Apex.Functions.getMoney(identifier, 'bank', function(bank)
        if bank >= amount then
            ...
            TriggerClientEvent('apx-money:notify', source, "You do not have enough money in your bank.")
        end
    end)
end
...
AddEventHandler('apx-money:deposit', function(amount)
    ...
    if amount and amount > 0 then
        depositMoney(identifier, amount)
    else
...
AddEventHandler('apx-money:withdraw', function(amount)
    ...
    amount = tonumber(amount)
    if amount and amount > 0 then
        withdrawMoney(identifier, amount)
    else
